...
use super::error::SendTimeoutError;
...
impl<T> Receiver<T> {
    ...
    /// `osaka::select!` statement and some other branch
    ...
    /// ```
    /// use osaka::sync::mpsc;
    /// use osaka::sync::mpsc::error::TryRecvError;
    ///
    /// #[osaka::main]
    /// async fn main() {
    ///     let (tx, mut rx) = mpsc::channel(100);
...
impl<T> Sender<T> {
    ...
    /// # Cancel safety
    ///
    /// If `send` is used as the event in a `osaka::select!`
    ...
    /// # Panics
    ///
    /// This function panics if it is called outside the context of a Osaka
    /// runtime with time enabled.
    ///
    /// # Examples
    ///
    ...
    pub fn blocking_send(&self, value: T) -> Result<(), SendError<T>> {
        crate::util::block_on(self.send(value))
    }
    ...
    ///
    /// ```
    /// let (tx, rx) = osaka::sync::mpsc::channel::<()>(42);
    /// assert!(!tx.is_closed());
    ///
    /// let tx2 = tx.clone();
    ...
    ///
    /// ```
    /// let (tx, rx) = osaka::sync::mpsc::channel::<()>(1);
    /// let  tx2 = tx.clone();
    /// assert!(tx.same_channel(&tx2));
    ///
    /// let (tx3, rx3) = osaka::sync::mpsc::channel::<()>(1);
