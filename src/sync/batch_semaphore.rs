...
impl Semaphore {
    ...
    pub fn new(permits: usize) -> Self {
        ...
        println!("state: {:b}", state.load(Relaxed));
    ...
    fn poll_acquire(
        ...
        println!(
            "[Semaphore] polling for {} needed {:b}",
            num_permits, needed
        );
        ...
        let mut waiters = loop {
            ...
            println!("remaining: {} next: {:b} acq: {}", remaining, next, acq);

            if remaining > 0 && lock.is_none() {
                println!("[Semaphore] Lock retry");
            ...
            match self.state.compare_exchange(curr, next, AcqRel, Acquire) {
                Ok(_) => {
                    ...
                    if remaining == 0 {
                        if !queued {
                            println!("[Semaphore] Accquired directly");
                            return Ready(Ok(()));
        ...
        if node.assign_permits(&mut acquired) {
            ...
            println!("[Semaphore] Aquired permits");
            return Ready(Ok(()));
        }
        ...
        if !queued {
            println!("[Semaphore] Enqueue waiter");
            let node = unsafe {
        ...
        println!("[Semaphore] Remain pending");
        Pending
    }
}
