...
use crate::adapter::{Mutex, UnsafeCell};
use crate::util::{Link, LinkedList, Pointers};
...
impl<T> Sender<T> {
    ...
    fn send2(&self, value: Option<T>) -> Result<usize, SendError<Option<T>>> {
        ...
        slot.rem.store(rem, std::sync::atomic::Ordering::Relaxed) /*with_mut(|v| *v = rem)*/;
...
unsafe impl Link for Waiter {
    ...
    unsafe fn pointers(mut target: NonNull<Waiter>) -> NonNull<Pointers<Waiter>> {
