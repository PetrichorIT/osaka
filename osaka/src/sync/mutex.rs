...
///
/// Osaka's Mutex works in a simple FIFO (first in, first out) style where all
...
impl<T: ?Sized> Mutex<T> {
    ...
    ///     let blocking_task = osaka::task::spawn_blocking(move || {
    ...
    /// ```
    /// use osaka::sync::Mutex;
    /// # async fn dox() -> Result<(), osaka::sync::TryLockError> {
    ///
    /// let mutex = Mutex::new(1);
    ///
