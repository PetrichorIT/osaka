...
//! ```
//!
//! To use a oneshot channel in a `osaka::select!` loop, add `&mut` in front of
//! the channel.
//!
//! *NOTE* Osaka does not support macro::main OR time primitves
//! ```ignore
//! use osaka::sync::oneshot;
//! use osaka::time::{interval, sleep, Duration};
//!
//! #[osaka::main]
//! # async fn _doc() {}
...
/// ```
///
/// To use a `Receiver` in a `osaka::select!` loop, add `&mut` in front of the
/// channel.
///
/// *NOTE* Osaka does not support macro::main OR time primitves
/// ```ignore
...
impl<T> Sender<T> {
    ...
    /// Paired with select
    ///
    /// *NOTE* That this wont complie since Osaka does not provide time primives now.
    /// ```ignore
    /// use osaka::sync::oneshot;
    /// use osaka::time::{self, Duration};
    ///
...
impl<T> Receiver<T> {
    ...
    /// ```
    /// use osaka::sync::oneshot;
    /// use osaka::sync::oneshot::error::TryRecvError;
    ///
    /// #[osaka::main]
    /// async fn main() {
    ...
    /// }
    /// ```
    #[deprecated = "Missing reimplementation"]
    pub fn blocking_recv(self) -> Result<T, RecvError> {
        crate::util::block_on(self)
    }
}
...
impl<T> Future for Receiver<T> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let ret = if let Some(inner) = self.as_ref().get_ref().inner.as_ref() {
            let res = match inner.poll_recv(cx)? {
                Ready(r) => r,
