...
impl<T> Receiver<T> {
    ...
    ///
    /// ```
    /// let (tx, rx) = osaka::sync::watch::channel(true);
    /// let rx2 = rx.clone();
    /// assert!(rx.same_channel(&rx2));
    ///
    /// let (tx3, rx3) = osaka::sync::watch::channel(true);
...
impl<T> Sender<T> {
    ...
    ///
    /// ```
    /// let (tx, rx) = osaka::sync::watch::channel(());
    /// assert!(!tx.is_closed());
    ///
    /// drop(rx);
    /// assert!(tx.is_closed());
    ...
    ///         # if false {
    ///         osaka::time::sleep(Duration::from_millis(10)).await;
    ///         # }
    ///         tx.send(100).unwrap();
    ///     });
    ///
