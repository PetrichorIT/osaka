...
const MAX_READS: usize = (std::u32::MAX >> 3) as usize;
...
///
/// The priority policy of Osaka's read-write lock is _fair_ (or
...
impl<T: ?Sized> RwLock<T> {
    ...
    #[track_caller]
    pub fn with_max_readers(value: T, max_reads: usize) -> RwLock<T>
    where
        T: Sized,
    {
        assert!(
    ...
    ///
    ///     let blocking_task = osaka::task::spawn_blocking({
    ...
    pub fn blocking_write(&self) -> RwLockWriteGuard<'_, T> {
        crate::util::block_on(self.write())
    }
