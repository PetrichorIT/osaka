...
/// `Result` because panics in the spawned task are caught by Osaka. The `?` operator has
...
///     let join_handle: task::JoinHandle<Result<i32, io::Error>> = osaka::spawn(async {
///         Ok(5 + 3)
///     });
///
...
impl<T> JoinHandle<T> {
    ...
    ///
    ///    handles.push(osaka::spawn(async {
    ...
    /// # async fn main() {
    /// # time::pause();
    /// let handle1 = osaka::spawn(async {
    ///     // do some stuff here
    /// });
    /// let handle2 = osaka::spawn(async {
