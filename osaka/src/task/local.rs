...
/// }
/// ```
/// **Note:** The `run_until` method can only be used in `#[osaka::main]`,
/// `#[osaka::test]` or directly inside a call to `Runtime::block_on`. It
/// cannot be used inside a task spawned with `osaka::spawn`.
///
/// ## Awaiting a `LocalSet`
///
...
/// `#[osaka::main]`, `#[osaka::test]` or directly inside a call to
/// `Runtime::block_on`. It cannot be used inside a task spawned with
/// `osaka::spawn`.
///
/// ## Use inside `osaka::spawn`
///
/// The two methods mentioned above cannot be used inside `osaka::spawn`, so
/// to spawn `!Send` futures from inside `osaka::spawn`, we need to do
...
/// *NOTE* No BUILDER
/// ```ignore
/// use osaka::runtime::Builder;
/// use osaka::sync::{mpsc, oneshot};
/// use osaka::task::LocalSet;
///
...
///                     osaka::task::spawn_local(run_task(new_task));
///                 }
...
struct Context {
    ...
    queue: VecDequeCell<task::Notified>,
    ...
    F::Output: 'static,
{
    CURRENT.with(|maybe_cx| {
        let cx = maybe_cx.expect("`spawn_local` called from outside of a `task::LocalSet`");
...
impl Context {
        ...
        F::Output: 'static,
    {
        ...
        unimplemented!("Context != Context huh")
        // let (handle, notified) = self.owned.bind(future, self.shared., id);
